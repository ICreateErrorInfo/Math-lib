...
namespace Raytracing.Tests {
    ...
    internal class BVHTests
    {
        ...
        {
            ISpectrum spectrum = new SampledSpectrum();
            Raytracer r = new Raytracer(null, null, null, spectrum);
            r.Init();
            List<Primitive> primitives = new List<Primitive>();

            var checker = new CheckerTexture(spectrum.FromRGB(new double[] {.2, .3, .1 }, SpectrumMaterialType.Reflectance), spectrum.FromRGB(new double[] {.9, .9, .9 }, SpectrumMaterialType.Reflectance));

            var material2 = new Metal(spectrum.FromRGB(new double[] {0.90, 0.76, 0.46}, SpectrumMaterialType.Reflectance), 0);
            var material3 = new Metal(spectrum.FromRGB(new double[] {0.65, 0.77, 0.97}, SpectrumMaterialType.Reflectance), 0);
            var material4 = new Metal(spectrum.FromRGB(new double[] {0.90, 0.90, 0.90}, SpectrumMaterialType.Reflectance), 0);
