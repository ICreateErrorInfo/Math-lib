...
namespace Projection
{
    class GeometryFlatEffect : Effect
    {
        ...
        {
            var n = Vector3D.UnitVector(Vector3D.Cross(v1.pos - v0.pos, v2.pos - v0.pos));
            var d = diffuse * Math.Max(0, Vector3D.Dot(-n , dir));
            var c = (color * (d + ambient)).Saturate() * 255;

            v0.col = Color.FromArgb((int)c.X, (int)c.Y, (int)c.Z);
            v1.col = Color.FromArgb((int)c.X, (int)c.Y, (int)c.Z);
            v2.col = Color.FromArgb((int)c.X, (int)c.Y, (int)c.Z);
